...
let state = {
  ...
  jobEditForms: {},
  selectedJobs: [],
  sourceForm: { label: "", url: "", region: "Remote / MENA" },
  scanResult: null,
  action: {
    pending: "",
    message: "",
...
function setupEvents() {
  ...
  document.querySelectorAll("[data-job]").forEach((item) => {
    item.addEventListener("click", (event) => {
      if (event.target.closest("[data-select-job]")) return;
  ...
  document.querySelectorAll("[data-select-job]").forEach((item) => {
    item.addEventListener("change", () => {
      const jobId = item.dataset.selectJob;
      state.selectedJobs = item.checked
        ? [...new Set([...state.selectedJobs, jobId])]
  ...
  document.querySelectorAll("[data-bulk-status]").forEach((item) => {
    item.addEventListener("click", async () => {
      const updated = await runAction(
        ...
        "تم تحديث الوظائف المحددة",
        () =>
          apiJson("/api/jobs/bulk-status", {
            method: "PATCH",
            body: JSON.stringify({ ids: state.selectedJobs, status: item.dataset.bulkStatus }),
          }),
      );
      if (updated) {
        updated.forEach(replaceJob);
        state.selectedJobs = [];
      }
      render();
    });
  ...
  document.querySelectorAll("[data-source-form-field]").forEach((item) => {
    item.addEventListener("input", (event) => {
      state.sourceForm[event.target.dataset.sourceFormField] = event.target.value;
    });
  });
  ...
  if (addSource) {
    addSource.addEventListener("click", async () => {
      const source = await runAction(
        "add-source",
        "جاري حفظ مصدر الوظائف...",
        "تمت إضافة المصدر",
        () => apiJson("/api/sources", { method: "POST", body: JSON.stringify(state.sourceForm) }),
      );
      if (source) {
        state.sources = [...state.sources.filter((item) => item.id !== source.id), source];
        state.importPreset = source.id;
        state.sourceForm = { label: "", url: "", region: "Remote / MENA" };
      }
      render();
    });
  }
  ...
  document.querySelectorAll("[data-scan-source]").forEach((item) => {
    item.addEventListener("click", async () => {
      const sourceId = item.dataset.scanSource;
      const source = state.sources.find((candidate) => candidate.id === sourceId);
      const result = await runAction(
        `scan-${sourceId}`,
        "جاري فحص المصدر واستخراج الوظائف المناسبة...",
        "تم تحديث الوظائف من المصدر",
        () =>
          apiJson(`/api/sources/${sourceId}/scan`, {
            method: "POST",
            body: JSON.stringify({
  ...
  shell(`${topbar()}
    <section class="hero-grid">
      <div class="command-panel">
        ...
        ${selectedCount ? `<div class="bulk-toolbar"><strong>${selectedCount} وظائف محددة</strong><span>نقلها إلى:</span><button class="secondary-btn" data-bulk-status="processing">قيد المعالجة</button><button class="secondary-btn" data-bulk-status="ready">جاهزة</button><button class="secondary-btn" data-bulk-status="applied">تم التقديم</button><button class="ghost-btn" data-bulk-status="in_progress">متابعة</button></div>` : `<p class="selection-hint">حدد أكثر من وظيفة لتحديث مسارها ومراقبتها معاً.</p>`}
...
function jobCard(job) {
  return `<article class="job-card" data-job="${job.id}">
    <label class="job-select" title="تحديد الوظيفة للمراقبة"><input type="checkbox" data-select-job="${job.id}" ${state.selectedJobs.includes(job.id) ? "checked" : ""} /><span></span></label>
...
function renderSources() {
  ...
  shell(`${topbar(copy.title)}
    <section class="detail-panel">
      ...
      <article class="assistant-card source-manager">
        <div class="section-title"><div><h2>أضف موقع وظائف جديد</h2><p>أدخل رابط موقع الوظائف ليظهر كمصدر مستقل ويمكن فحصه وربط نتائجه بملفك المهني.</p></div></div>
        <div class="import-grid">
          <div class="field"><label>اسم الموقع</label><input data-source-form-field="label" value="${escapeHtml(sourceForm.label)}" placeholder="Remote OK أو موقع محلي" /></div>
          <div class="field"><label>رابط الموقع أو صفحة البحث</label><input data-source-form-field="url" value="${escapeHtml(sourceForm.url)}" placeholder="https://example.com/jobs" inputmode="url" /></div>
          <div class="field"><label>المنطقة</label><input data-source-form-field="region" value="${escapeHtml(sourceForm.region)}" placeholder="MENA أو عن بعد" /></div>
        </div>
        <div class="button-row"><button class="primary-btn" data-add-source>${icon("source")} إضافة المصدر</button></div>
      </article>
      ${state.scanResult ? `<div class="import-review scan-result"><strong>تم فحص ${escapeHtml(state.scanResult.source.label)}</strong><p class="screen-note">أضيفت ${state.scanResult.jobs.length} وظائف مناسبة إلى قائمة المتابعة. يمكنك تحديد أكثر من وظيفة من لوحة البحث.</p><span class="small-chip teal">${state.scanResult.mode}</span></div>` : ""}
      <article class="assistant-card import-card">
