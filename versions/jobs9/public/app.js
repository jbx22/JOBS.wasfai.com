...
function renderHowItWorks(scope = "home") {
  const steps = [
    ...
    },
    {
      ...
      text: "كاتب الظل ينتج سيرة عربية، سيرة إنجليزية، خطابين، وأسئلة مقابلة لكل وظيفة.",
      action: "كاتب الظل",
      route: "/jobs",
...
function renderJobDetail(id) {
  ...
  const resume = `<div class="tab-panel">
    <div class="section-block">
      ...
      <div class="row">
        <button class="btn primary" data-ghostwriter="${esc(job.id)}">${iconSpark()} كاتب الظل</button>
...
function renderGhostwriterPanel(job, kit) {
  ...
  const provider = kit?.provider === "deepseek" ? "DeepSeek" : kit?.provider === "ai" ? "AI" : kit?.provider === "template" ? "قالب ذكي" : "جاهز للتوليد";
  ...
  return `<div class="section-block ghostwriter-panel">
    <div class="section-head tight">
      <div>
        <h3>كاتب الظل للمشترك</h3>
        <p class="muted tiny">سيرة وخطاب ومقابلة بالعربية والإنجليزية لكل وظيفة.</p>
      </div>
    ...
    ${
      hasKit
        ...
        : `<div class="empty compact">اضغط كاتب الظل لتجهيز نسخة عربية وإنجليزية مخصصة لهذه الوظيفة، مع أسئلة مقابلة وإجابات تدريبية.</div>`
    }
  </div>`;
}
      ...
      const kit = await runAction(
        ...
        () =>
          apiJson("/api/ghostwriter", {
            method: "POST",
            body: JSON.stringify({ job, profile: state.profile }),
          }),
      );
      if (kit) {
