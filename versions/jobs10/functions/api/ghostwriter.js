...
const AI_WRITER_MODELS = {
  ...
  "minimax-m3": {
    ...
    baseUrlNames: ["MINIMAX_BASE_URL"],
    modelNames: ["MINIMAX_MODEL"],
  ...
  "glm-5.2": {
    provider: "glm",
    label: "GLM 5.2",
    keyNames: ["GLM_API_KEY", "ZAI_API_KEY", "ZHIPU_API_KEY"],
    baseUrlNames: ["GLM_BASE_URL", "ZAI_BASE_URL", "ZHIPU_BASE_URL"],
    modelNames: ["GLM_MODEL", "ZAI_MODEL", "ZHIPU_MODEL"],
  ...
  "kimi-2.7": {
    ...
    baseUrlNames: ["KIMI_BASE_URL", "MOONSHOT_BASE_URL"],
    modelNames: ["KIMI_MODEL", "MOONSHOT_MODEL"],
...
async function maybeGenerateWithAi(env, job, profile, writer) {
  const apiKey = writer.apiKey || env.OPENAI_API_KEY;
  if (!apiKey) return null;

  const baseUrl = (writer.baseUrl || OPENAI_BASE_URL).replace(/\/+$/, "");
  const model = writer.model || OPENAI_FALLBACK_MODEL;
