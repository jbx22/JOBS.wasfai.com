 ...
 *
 * If OPENAI_API_KEY is configured, this calls an OpenAI-compatible chat
 * endpoint. Without credentials it returns a deterministic, structured draft
 * so the subscriber workflow still works in production.
 */
...
export async function onRequestPost(context) {
  try {
    ...
    return json({
      ...
      provider: aiResult ? "ai" : "template",
      model: aiResult ? env.AI_MODEL || DEFAULT_MODEL : "local-template",
      ...kit,
      checklist: [
...
async function maybeGenerateWithAi(env, job, profile) {
  ...
  try {
    return validateKit(JSON.parse(text));
  } catch {
    return null;
  }
}
