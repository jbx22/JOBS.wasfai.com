...
- If `OPENAI_API_KEY` or `AI_API_KEY` is configured in Cloudflare Pages environment variables, the route calls an OpenAI-compatible chat endpoint. Without credentials, it returns a structured deterministic kit so the production workflow remains usable.
