  ...
  return `<header class="topbar">
    ...
    <div class="user-menu-wrap">
      ...
      <div class="user-menu" data-user-menu-panel hidden>
        <button data-toggle-locale>${esc(tr("language"))}</button>
        <button data-toggle-theme>${state.theme === "dark" ? esc(tr("themeLight")) : esc(tr("themeDark"))}</button>
        <button data-nav="${state.session?.authenticated ? "/account" : "/login"}">${iconUser()} ${state.session?.authenticated ? "الحساب" : "تسجيل الدخول / إنشاء حساب"}</button>
      </div>
    </div>
  </header>`;
}
