...
export async function requireFileStorage(context) {
  ...
  if (!getJobDb(context.env) || !context.env?.RESUME_FILES || !context.env?.FILE_ENCRYPTION_KEY) {
    return { error: json({ error: "Private résumé storage is not configured.", code: "STORAGE_NOT_CONFIGURED" }, 503) };
  }
  await getJobDb(context.env).prepare(FILE_TABLE_SQL).run();
  await getJobDb(context.env).prepare(FILE_INDEX_SQL).run();
  const account = await getJobDb(context.env).prepare("SELECT account_status FROM users WHERE id = ?").bind(user.sub).first();
  ...
  return { user, db: getJobDb(context.env), bucket: context.env.RESUME_FILES };
}
