...
function sourceCardHtml(s) {
  ...
  <div class="kvs" style="margin-top:6px">
    ...
    <div class="right">
      ...
      ${s.connector_mode === "public_html" ? `<label class="toggle"><input type="checkbox" data-source-schedule="${esc(s.id)}" ${s.scheduled ? "checked" : ""}/><span class="slider"></span></label>
      ...
      <div class="section-block">
        ...
        <div class="form profile-form">
          ...
          <div class="row profile-actions">
            <button class="btn outline" data-profile-autofill>${iconSpark()} تعبئة البيانات من السيرة</button>
...
function renderAuthPanel() {
  ...
  return `<div class="section-block auth-panel">
    ...
    ` : `
      <div class="row profile-actions">
        <a class="btn primary" href="/api/auth/google/start">${iconUser()} المتابعة بحساب Google</a>
...
function setupEvents() {
  ...
  document.querySelectorAll("[data-resume-file]").forEach((resumeFile) => {
    resumeFile.addEventListener("change", async (e) => {
      ...
      if (!readableResumeText(text)) {
        ...
        state.action = { pending: "", message: "", error: "لم أستطع قراءة نص واضح من الملف. إذا كان PDF صورة أو سكان، انسخ نص السيرة والصقه في مربع السيرة أو ارفع ملف PDF نصي." };
      } else {
  ...
  if (autofillProfile) {
    autofillProfile.addEventListener("click", () => {
      ...
      applyResumeSignalsFromText(text);
      saveLocalProfile(state.profile);
      state.action = { pending: "", message: "تمت تعبئة البيانات من السيرة", error: "" };
      render();
    });
  }
  ...
  if (approveUploadedResume) {
    approveUploadedResume.addEventListener("click", () => {
      ...
      state.approvedMasterResume = true;
      const relevantCount = resumeRelevantJobs(state.jobs).length;
      if (relevantCount < 3) {
        const existing = new Map(state.jobs.map((job) => [job.id, job]));
        for (const job of fallbackMatchedJobsFromProfile()) existing.set(job.id, { ...(existing.get(job.id) || {}), ...job });
        state.jobs = [...existing.values()].sort((a, b) => Number(b.score || 0) - Number(a.score || 0));
      }
