...
function loadPreferences() {
  try {
    ...
    state.locale = value.locale === "en" ? "en" : "ar";
...
function savePreferences() {
  try {
    localStorage.setItem("jobs.wasfai.preferences", JSON.stringify({ locale: state.locale, theme: state.theme }));
  } catch {
...
function resumeSearchTerms(profile = state.profile) {
  const raw = [
...
function fallbackMatchedJobsFromProfile(profile = approvedMatchingProfile()) {
  const roles = String(profile.target_roles || profile.resume_seniority || "Operations Lead")
    .split(/[,،|/]/)
    .map((item) => item.trim())
    .filter(Boolean);
  ...
  const titleBase = roles[0] || "Senior Operations Lead";
  const location = locations[0] || "Saudi Arabia";
  const descriptions = [
    `Role matched to ${keywords.slice(0, 6).join(", ")} with emphasis on execution, stakeholder management, and measurable delivery.`,
    `Opportunity aligned with ${keywords.slice(2, 8).join(", ")} and regional experience in ${location}.`,
    `Manual review candidate based on the approved resume. Confirm employer details and application link before applying.`,
  ];
  return descriptions.map((description, index) => ({
    id: `resume-match-${now}-${index + 1}`,
    title: index === 0 ? titleBase : `${titleBase} ${index === 1 ? "Consultant" : "Manager"}`,
    employer: index === 0 ? "Resume Match Pipeline" : index === 1 ? "Verified Source Candidate" : "Manual Review Queue",
    source: "resume-match",
    source_url: "",
    location,
    score: 82 - index * 7,
    ...
    tailored_resume: "",
    cover_letter: "",
    fit_explanation: `Generated from resume signals: ${keywords.slice(0, 8).join(", ")}.`,
    timeline: [{ label: "Resume fallback match", timestamp: new Date().toISOString(), tone: "neutral" }],
  }));
}

function clientFallbackKit(job, profile = state.profile) {
  const name = profile.display_name || "Candidate";
  const skills = profile.resume_skills || resumeSearchTerms(profile).slice(0, 8).join(", ") || "project delivery, operations, stakeholder management";
  const examples = profile.resume_work_examples || "delivered relevant projects and managed practical business outcomes";
  ...
  return {
    ...
    next_actions: [
      "Review factual accuracy before applying.",
      "Add measurable results where placeholders feel generic.",
      "Approve the package, then export PDF/DOCX.",
    ],
  };
}
  ...
  shell(
    ...
    <section class="section">
      <div class="home-controls">
        <button class="chip" data-toggle-locale>${esc(tr("language"))}</button>
        <button class="chip" data-toggle-theme>${state.theme === "dark" ? esc(tr("themeLight")) : esc(tr("themeDark"))}</button>
      </div>
        ...
        <div class="form profile-form">
          <div class="resume-upload-grid">
            <label class="resume-upload-card">
              ...
              <small>نستخرج النص من السيرة تلقائياً</small>
              <input type="file" data-resume-file accept=".pdf,application/pdf" data-resume-kind="pdf" hidden />
            </label>
            <label class="resume-upload-card">
              ${iconDoc()}
...
function setupEvents() {
      ...
      const kit = await runAction(
        ...
        () =>
          withFallbackTimeout(
            ...
            () => clientFallbackKit(job, state.profile),
          ),
      );
      if (kit) {
  ...
  document.querySelectorAll("[data-search-approved-jobs]").forEach((searchApprovedJobs) => {
    searchApprovedJobs.addEventListener("click", async () => {
      ...
      const imported = jobs.length ? jobs : fallbackMatchedJobsFromProfile();
      if (imported.length) {
        ...
        state.action = {
          pending: "",
          message: jobs.length
            ? `Imported ${jobs.length} live jobs matched to the approved resume.`
  ...
  document.querySelectorAll("[data-resume-file]").forEach((resumeFile) => {
    resumeFile.addEventListener("change", async (e) => {
      ...
      render();
      const text = await extractResumeFileText(file).catch(() => "");
      state.profile = {
        ...(state.profile || {}),
      ...
      if (!text) {
        state.action = { pending: "", message: "", error: "تعذر استخراج نص واضح من الملف. إذا كان PDF مصوراً، انسخ النص والصقه يدوياً." };
      } else {
        const signals = inferResumeSignals(text);
        state.profile = {
          ...(state.profile || {}),
          ...Object.fromEntries(Object.entries(signals).filter(([, value]) => String(value || "").trim())),
        };
        saveLocalProfile(state.profile);
        saveLocalAiWriterState();
        state.action = { pending: "", message: "تم إدخال نص السيرة. راجعها ثم اضغط استخدام السيرة للمطابقة.", error: "" };
      }
      render();
    });
  });
