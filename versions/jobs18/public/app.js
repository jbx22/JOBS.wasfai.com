...
function saveWorkspaceState() {
  ...
  saveWorkspaceTimer = setTimeout(async () => {
    try {
      ...
      if (response.ok) {
        state.persistence = { storage: "d1", pending: false, last_saved_at: new Date().toISOString() };
      }
    } catch {
...
function renderCommandHome() {
  ...
  const approvedSources = sources.filter((s) => s.connector_mode === "approved_api").length;
  const publicSources = sources.filter((s) => s.connector_mode === "public_html").length;
  ...
  const actionCards = [
    ["Complete Profile", ready ? "Approved resume is ready for matching." : "Paste the full resume, improve it, and approve the master profile.", "/account", iconUser(), ready ? "Ready" : `${profileCompleteness()}%`, ready],
    ["Add Sources", "Choose approved APIs, public HTML boards, or manual paste/import.", "/settings", iconCog(), `${sources.length}`, sources.length > 0],
    ["Review Matches", ready ? "Open ranked jobs and move them through the pipeline." : "Strong match scores unlock after profile approval.", "/jobs", iconList(), ready ? `${state.jobs.length}` : "Locked", ready && state.jobs.length > 0],
    ["Generate Package", "Create bilingual CV, cover letter, interview prep, then export.", "/jobs", iconSpark(), `${generatedPackages}`, generatedPackages > 0],
  ]
    .map(
  ...
  const selectedHtml = matched
    .map(
      (j) => `<div class="item" data-job="${esc(j.id)}">
        <div class="score-ring" style="--score:${j.score}"><div style="display:grid;place-items:center;line-height:1"><span style="font-size:18px;font-weight:800;color:var(--teal-2)">${j.score}</span><small style="font-size:9px;color:var(--muted)">match</small></div></div>
  ...
  shell(
    ...
    <section class="section">
      <div class="search">${iconSearch()}<input data-search value="${esc(state.query)}" placeholder="Search jobs" /><span class="kbd">⌘ K</span></div>
    ...
    <section class="section">
      <div class="mobile-command">
        <div>
          <span class="eyebrow">Mobile workflow</span>
          <h2>Four steps from resume to tracked application</h2>
          <p>${ready ? "Your approved profile is active. Review matches and generate packages." : "Approve the master resume first so matching is based on real profile data."}</p>
        </div>
        <span class="badge ${ready ? "" : "gold"}">${ready ? "Profile approved" : "Profile required"}</span>
      </div>
    </section>
    <section class="section"><div class="action-grid">${actionCards}</div></section>
    ${ready ? "" : `<section class="section"><div class="onboarding-gate">
      <strong>Matching is locked until onboarding is complete</strong>
      <p>Scores are hidden from the main journey until the original resume is improved and approved.</p>
      <button class="btn primary" data-nav="/account">${iconUser()} Complete Profile</button>
    </div></section>`}
    <section class="section">
      <div class="section-head"><h2>Next matches</h2><button class="more" data-nav="/jobs">Open</button></div>
      <div class="selected-jobs">${selectedHtml || `<p class="empty">${ready ? "Run a source scan to bring in matched jobs." : "Complete Profile first. Then this area shows your best matches."}</p>`}</div>
    </section>
    <section class="section">
      <div class="section-head"><h2>Pipeline snapshot</h2><button class="more" data-nav="/results">Details</button></div>
      <div class="status-grid">
        <div class="stat teal"><span class="v">${state.jobs.length}</span><span class="l">Jobs</span></div>
        <div class="stat"><span class="v">${counts.ready || 0}</span><span class="l">Ready</span></div>
        <div class="stat"><span class="v">${counts.in_progress || 0}</span><span class="l">Follow-up</span></div>
        <div class="stat gold"><span class="v">${approvedSources}/${publicSources}</span><span class="l">API/HTML</span></div>
      </div>
      <div class="persistence-note">
        <span>${state.session?.authenticated ? "Signed-in D1 workspace" : "Guest device workspace"}</span>
        <span>${state.persistence?.pending ? "Saving..." : state.persistence?.storage === "d1" ? "Durable" : "Local"}</span>
      </div>
    </section>`,
    { screen: "search" },
  );
...
function jobCardHtml(job) {
  ...
  return `<article class="card job-card ${selected ? "selected" : ""}" data-job="${esc(job.id)}">
    <div class="score-ring ${ready ? tone : "locked"}" style="--score:${ready ? job.score : 0}"><span>${ready ? `${job.score}%` : "Profile"}</span></div>
    <div class="body">
      <div class="row1">
...
function renderApplicationPackageBlock(job, approved) {
  return `<div class="application-package ${approved ? "ready" : ""}">
    <div>
      ...
      <strong>${approved ? "Ready to apply" : "Approve first, then download"}</strong>
      <p class="muted tiny">Download the approved PDF for upload. DOCX is available when you need to edit before applying.</p>
    </div>
    <div class="package-actions">
      <button class="btn gold full" data-export-kit="pdf" data-job-id="${esc(job.id)}" ${approved ? "" : "disabled"}>${iconDoc()} Download PDF to Apply</button>
      <button class="btn outline" data-export-kit="docx" data-job-id="${esc(job.id)}" ${approved ? "" : "disabled"}>${iconDoc()} DOCX</button>
      <button class="btn outline" data-copy-ghostwriter="${esc(job.id)}">${iconDoc()} Copy</button>
    </div>
  </div>`;
}
...
function renderAssistedApplyPanel(job, pkg, draft, kit) {
  ...
  return `<div class="section-block assisted-apply">
    <div class="section-head tight">
      <div>
        <h3>Assisted Apply + Follow-up</h3>
        <p class="muted tiny">Prepare, review, open the job board, then submit only after your confirmation.</p>
      </div>
      <span class="badge ${followup ? "" : "muted"}">${followup ? "Follow-up" : submitted ? "Submitted" : "Not submitted"}</span>
    </div>
    <div class="checklist" data-application-checklist="${esc(job.id)}">${buildChecklist(job, pkg, draft)}</div>
    <div class="apply-guard">
      <strong>No blind auto-apply</strong>
      <p class="muted tiny">The app can help fill easy boards and track the result, but final submit stays with you.</p>
    </div>
    <div class="apply-actions">
      <button class="btn primary" data-export-kit="pdf" data-job-id="${esc(job.id)}" ${approved ? "" : "disabled"}>${iconDoc()} Download PDF to Apply</button>
      ${applyUrl ? `<a class="btn outline" href="${esc(applyUrl)}" target="_blank" rel="noopener">${iconGlobe()} Open Job Post</a>` : `<button class="btn outline" disabled>${iconGlobe()} Job link missing</button>`}
      <button class="btn outline" data-submit-action="${esc(job.id)}" ${hasKit && approved && !submitted ? "" : "disabled"}>${iconCheck()} Mark Submitted</button>
      <button class="btn outline" data-followup-action="${esc(job.id)}" ${submitted ? "" : "disabled"}>${iconClock()} Track Follow-up</button>
    </div>
  </div>`;
}
...
function renderSettings() {
  ...
  const sourceWorkflow = `<section class="section">
    <div class="source-workflow">
      <div><strong>Approved APIs</strong><small>LinkedIn and Indeed stay locked until official provider access is added.</small></div>
      <div><strong>Public HTML</strong><small>Bayt, WUZZUF, Hiring Cafe, and similar boards can be scanned with limits.</small></div>
      <div><strong>Manual import</strong><small>Paste a job URL or description when a source blocks automated access.</small></div>
      <div><strong>Scan status</strong><small>Every source shows last scan, schedule, connector mode, and errors.</small></div>
    </div>
  </section>`;
  shell(
    `${banner()}
