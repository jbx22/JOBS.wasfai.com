  ...
  return withState(context, async (state) => {
    ...
    try {
      ...
      if (!scanResponse.ok || !scanPayload.ok) {
        const detail = scanPayload.error || `ingestion worker returned ${scanResponse.status}`;
