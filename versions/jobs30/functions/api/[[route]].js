...
async function sourceScan(context, id) {
  return withState(context, async (state) => {
    ...
    const jobs = [1, 2].map((n) => ({
      ...
      title: n === 1 ? "مدير تشغيل مناسب للسيرة" : "مهندس ميكانيكي أول",
      employer: source.label,
      source: id,
      source_url: source.url,
      location: source.region || "عن بعد",
      score: n === 1 ? 82 : 76,
      status: "discovered",
      deadline: "",
      description: `تمت الإضافة من ${source.label} بعد فحص المصدر وربطها بالسيرة المعتمدة.`,
      tailored_resume: "",
      cover_letter: "",
      fit_explanation: "تم حساب المطابقة مقابل بيانات السيرة المعتمدة.",
      timeline: [{ label: "فحص مصدر موثق", timestamp: new Date().toISOString(), tone: "neutral" }],
    }));
