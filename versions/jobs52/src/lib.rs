...
pub fn app_with_store(store: PersistentStore) -> Router {
    Router::new()
        ...
        .route("/render-pdf", post(post_render_pdf))
...
struct PdfRenderKit {
    ...
    en_resume: String,
    #[serde(default)]
    ar_cover_letter: String,
    #[serde(default)]
    en_cover_letter: String,
}
...
async fn post_render_pdf(Json(payload): Json<PdfRenderRequest>) -> Result<Response, StatusCode> {
    if payload.job.id.trim().is_empty()
        || payload.job.title.trim().is_empty()
        || payload.job.employer.trim().is_empty()
        || (payload.kit.ar_resume.trim().is_empty() && payload.kit.en_resume.trim().is_empty())
    {
        return Err(StatusCode::BAD_REQUEST);
    }
    let job = Job {
        id: payload.job.id.clone(),
        title: payload.job.title,
        employer: payload.job.employer,
        source: "application-kit".into(),
    ...
    let package = ApplicationPackage {
        ...
        resume_title: format!("{} — Bilingual CV", payload.profile.display_name),
        resume_body: format!("Arabic Resume\n{}\n\nEnglish Resume\n{}", payload.kit.ar_resume, payload.kit.en_resume),
        cover_letter_title: format!("Application — {}", job.employer),
        cover_letter_body: format!("Arabic Cover Letter\n{}\n\nEnglish Cover Letter\n{}", payload.kit.ar_cover_letter, payload.kit.en_cover_letter),
        pdf_status: "Approved bilingual package".into(),
        generated_at: current_millis().to_string(),
    };
    let bytes = render_package_pdf_bytes(&job, &package);
    Ok(Response::builder()
